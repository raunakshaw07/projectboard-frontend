...
import { updateProjectRoute } from '../utils/APIRoutes'
...
const EditProjectModal = ({ open, handleClose, project }) => {
    const [values, setValues] = useState({ title: project.title, desc: project.description });
    ...
    const handleValidation = () => {
        if (values.lane.trim() === '' || values.title.trim() === '' || values.desc.trim() === '')
            return false
        return true
    }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        if (handleValidation()) {
            const { data } = await axios.post(`${updateProjectRoute}/${project._id}`, { title: values.title, description: values.desc });
            if (data.status) {
                handleClose();
            } else {
    ...
    return (
        <div>
            ...
            >
                <Box sx={style.box}>
                    <Typography variant='h4' component='h4' sx={style.cardHead}>Edit Project</Typography>
                    <hr width-='90%' />
                    <form style={style.form} onSubmit={handleSubmit}>
                        ...
                        <CssTextField
                            ...
                            name='createdOn'
                            sx={{ color: '#eee' }}
                            value={moment(project.createdAt).format('MMMM DD, YYYY')}
                        />
                        <CssTextField
                            fullWidth
                            type='text'
